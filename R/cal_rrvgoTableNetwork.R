...
for(i in c(1:length(names(V(g))))) {
  ...
  pie.tmp = c(0,0,0,0)
  for(j in c(1:nrow(network.input))) {
    if(network.input$term[j] == term.tmp & network.input$tech[j] == 'CS')    { pie.tmp[1] = pie.tmp[1] + 1 }
    if(network.input$term[j] == term.tmp & network.input$tech[j] == 'hvCAS') { pie.tmp[2] = pie.tmp[2] + 1 }
    if(network.input$term[j] == term.tmp & network.input$tech[j] == 'hvCTS') { pie.tmp[3] = pie.tmp[3] + 1 }
    if(network.input$term[j] == term.tmp & network.input$tech[j] == 'hvCOC') { pie.tmp[4] = pie.tmp[4] + 1 }
    
    if(network.input$box[j] == term.tmp & network.input$tech[j] == 'CS')    { pie.tmp[1] = pie.tmp[1] + 1 }
    if(network.input$box[j] == term.tmp & network.input$tech[j] == 'hvCAS') { pie.tmp[2] = pie.tmp[2] + 1 }
    if(network.input$box[j] == term.tmp & network.input$tech[j] == 'hvCTS') { pie.tmp[3] = pie.tmp[3] + 1 }
    if(network.input$box[j] == term.tmp & network.input$tech[j] == 'hvCOC') { pie.tmp[4] = pie.tmp[4] + 1 }
  }
  pie.list[[i]] = pie.tmp
}
